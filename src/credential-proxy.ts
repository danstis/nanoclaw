...
export function startCredentialProxy(
  ...
  // OpenRouter takes priority when its API key is configured
  const apiKey = secrets.OPENROUTER_API_KEY || secrets.ANTHROPIC_API_KEY;
  const authMode: AuthMode = apiKey ? 'api-key' : 'oauth';
  const oauthToken =
    secrets.CLAUDE_CODE_OAUTH_TOKEN || secrets.ANTHROPIC_AUTH_TOKEN;

  const defaultBaseUrl = secrets.OPENROUTER_API_KEY
    ? 'https://openrouter.ai/api'
    : 'https://api.anthropic.com';
  const upstreamUrl = new URL(
    secrets.OPENROUTER_BASE_URL || secrets.ANTHROPIC_BASE_URL || defaultBaseUrl,
  );
